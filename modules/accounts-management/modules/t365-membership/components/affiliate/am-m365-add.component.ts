...
@Component({
  selector: 'byte-am-m365-add',
  templateUrl: './am-m365-add.component.html',
  styleUrls: ['./am-m365-add.component.scss']
})
export class AmM365AddComponent implements OnInit {
  ...
  executeTransaction(token?: string) {
    ...
    return this.transactionService.affiliate({
  ...
  handleResponseTransaction(response: any) {
    ...
    this.parameterManagement.sendParameters({
      ...
      [PROTECTED_PARAMETER_ROUTE]: M365MembershipProtectedParameters.AFFILIATE_VOUCHER,
    });

    this.router.navigate([ET365MUrlCollection.AFFILIATE_VOUCHER]).finally(() => {});
  }
